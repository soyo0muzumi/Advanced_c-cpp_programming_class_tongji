...
void calc_next(char d_cur[], char d_next[])
{
	...
	for (int i = 1; i <= N_POW; ++i) {
		if (d_cur[i] != 0) {
			d_next[i] = (mod * 10 + d_cur[i] - '0') / 2 + '0';
		}
		else {
			d_next[i] = (mod * 10) / 2 + '0';
			break;
		}
		mod = (mod * 10 + d_cur[i] - '0') % 2;
	}
}
...
/***************************************************************************
  ...
  输入参数：int pure_decimal[] : 存放二进制小数的数组
			int out[] ：存放计算得到的十进制小数
		   int power_table[][N_NUM + 1]：幂次表
  返 回 值：
  说    明：
 ***************************************************************************/
...
{
	for (int i = 1; i < N_POW; ++i) {
		if (pure_decimal[i] == 0) break;
		if (pure_decimal[i] - '0' == 1) {
			for (int j = 0; j < N_NUM + 1; ++j) {
				if (power_table[i][j] == 0) break;
...
{
	cout << "请输入一个二进制纯小数，小数点开头，小数点后不超过112位(例：.101101)\n";
	char puredecimal[N_NUM + 1] = { 0 };
	bool is_legal_input = true;
	while (is_legal_input) {
		...
		else cout << "不是以.开头，请重输\n";
		for (int i = 1; i <= N_NUM; ++i) {
			if (puredecimal[i] == 0) break;
			if (puredecimal[i] != '0' && puredecimal[i] != '1') {
				cout << "输入的不是0/1，请重输\n";
				is_legal_input = false;
				break;
			}
...
int main()
{
	...
	*		第[112]行 -112   ...
	*
	注：1、本二维数组的类型，既可以是int，也可以是int，因为计算时，每个元素的值都在0-9之间，用int足够了（int型输出时注意数字0-9和字符0-9的问题即可）
